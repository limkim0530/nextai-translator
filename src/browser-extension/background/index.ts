...
browser.runtime.onConnect.addListener(async function (port) {
    switch (port.name) {
        case BackgroundEventNames.fetch:
            ...
            port.onMessage.addListener(function (rawMessage) {
                // webextension-polyfill types port messages as `unknown`; this
                // port only ever carries the fetch protocol.
                const message = rawMessage as BackgroundFetchRequestMessage
                switch (message.type) {
                    case 'abort':
                        controller.abort()
                        break
